...
export default function Forex() {
    ...
    const [Forexdata, setForexdata] = useState<any | null>(data);
    ...
    useEffect(() => {
        async function fetchForexData(){
            try {
                ...
                setForexdata(parsedData.results);
                setIsLoading(false);
            } catch (error) {
        ...
        <View className={styles.container}>
            <StatusBar style={Platform.OS === 'ios' ? 'light' : 'auto'} />
                ...
                <ScrollView contentContainerStyle={{alignItems:"center",justifyContent:"center"}} className='w-full flex'>
                    ...
                    <TextInput className='p-2 rounded-lg flex-1 border h-14' placeholder='Enter Forex Name' value={name} onChangeText={(txt) => { setName(txt) }} />
                    <Pressable className='bg-blue-500 h-14 flex items-center justify-center px-4 text-white p-2 rounded-lg' onPress={() => { fetchForex() }} >
                        {!isLoading?(<FontAwesome color="white" name='search' />):(<Text>Loading..</Text>)}
                        </Pressable>
                </View>
                {Forexdata.map((item: any, idx: any) => (
